...
function displayTime() {
    console.log("Called displayTime()!")
    const input = getInput();
    const dateStr = getDateStr(input);

    const element = document.getElementById("output_time");
...
function getInput() {
    ...
    const time = document.getElementById("input_time").value.toString();
    const hour = Number(time.split(":")[0]);
    const min = Number(time.split(":")[1]);

    return {
        fromCity: document.getElementById("from_city").value.toString(),
        toCity: document.getElementById("to_city").value.toString(),
        day: day,
...
function getDateStr(input) {
    const fromZone = getTimeZone(input.fromCity);
    // dayjs.tz.setDefault(fromZone);

    const toZone = getTimeZone(input.toCity);
    const dateString = input.year.toString() + '-' + input.monthIndex.toString() + '-' + input.day.toString() + 
                        ' ' + input.hour.toString() + ':' + input.min.toString();
    console.log(input.monthIndex);
    console.log(dateString);
    // dayjs.tz("2014-06-01 12:00", toZone);
    return new Date(input.year, input.monthIndex, input.day, input.hour, input.min)
        .toLocaleString("en-CA", {timeZone: tz});
}

function getTimeZone(city) {
    for (let zone of TIME_ZONES) {
        if (zone.includes(city)) {
            return zone;
        }
    }
}
